...
int op_reformat() {
    ...
    nano_fs_reformat();
    nano_fs_stat(filename, filename_len, &fileInfo);
    TEST_ASSERT_EQUAL_UINT8(0, fileInfo.write_times);
    TEST_ASSERT_EQUAL_UINT8(0, fileInfo.stat_times);
}

TEST_GROUP(test_reformat);

TEST_SETUP(test_reformat) {
        fake_storage_init();
        nano_fs_init(NULL, 122 * 4 * 1024, 4 * 1024, 6, 0);
}

TEST_TEAR_DOWN(test_reformat) {
        nano_fs_destroy();
}

TEST(test_reformat, op_reformat
) {
op_times();
...
TEST_GROUP_RUNNER(test_reformat) {
        RUN_TEST_CASE(test_reformat, op_reformat);
}